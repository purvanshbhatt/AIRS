...
class RecoveryReadinessCapability(BaseCapability):
    ...
    def evaluate(cls, evidence: List[Evidence]) -> List[EvaluationResult]:
        ...
        backup_evidence = [e for e in evidence if e.kind == EvidenceKind.BACKUP_STATUS]
        if not backup_evidence:
             return [EvaluationResult(
                 verdict=Verdict.UNKNOWN,
                 details={'system_name': 'Backup System', 'missing_data': True}
             )]
    ...
    def translate(cls, result: EvaluationResult) -> MomentTranslation:
        ...
        if details.get('missing_data'):
            return MomentTranslation(
                what_happened="Cannot verify if your backups are running.",
                why_care="Without monitoring, your backup system could fail silently, leaving you unable to recover from ransomware.",
                ignore_impact="You may find out your backups have been failing for months only when you actually need them."
            )
