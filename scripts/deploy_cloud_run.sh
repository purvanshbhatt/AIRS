...
# Safety: block production demo deploy unless explicitly allowed
IS_PROD_TARGET="false"
if [[ -n "$PROJECT_ID" && "$PROJECT_ID" == "$PROD_PROJECT_ID" ]]; then
    IS_PROD_TARGET="true"
elif [[ -z "$PROJECT_ID" && "$SERVICE_NAME" == "airs-api" ]]; then
    IS_PROD_TARGET="true"
elif [[ -z "$PROJECT_ID" && "$ENV_FILE" == *"env.prod"* ]]; then
    IS_PROD_TARGET="true"
fi

if [[ "$IS_PROD_TARGET" == "true" && "$ALLOW_PROD" != "true" ]]; then
